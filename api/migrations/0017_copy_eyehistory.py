...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for item in orm.EyeHistory.objects.all():
            try:
                dup_raw_histories = orm.EyeHistoryRaw.objects.filter(user=item.user, 
                                                      url=item.url, 
                                                      title=item.title, 
                ...
                if len(dup_raw_histories) == 0:
                    raw = orm.EyeHistoryRaw()
                    raw.user = item.user
                    raw.url = item.url
                    raw.title = item.title
                    raw.start_event = item.start_event
                    raw.end_event = item.end_event
                    raw.start_time = item.start_time
                    raw.end_time = item.end_time
                    raw.src = item.src
                    raw.domain = item.domain
                    raw.favIconUrl = item.favIconUrl
                    raw.total_time = item.total_time
                    raw.humanize_time = item.humanize_time
    ...
    models = {
        'api.blacklistitem': {
            ...
            'date_created': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2013, 12, 12, 0, 0)'}),
