...
function setupFilterList() {
    setupTemplateValues(mutelistFilterset, addFilterlist, "mutelist");
    setupTemplateValues(mutelistFiltersetWord, addFilterlist2, "mutelist");
}
...
function addFilterlist(res, type) {
    if (res.success && type === "mutelist") {
        ...
        var template;
        if ($(".add-word").val() !== "") {
            template = ich[TEMPLATE_BASE + "mutelist_set_row2.html"](data);
            $(".add-word").val("");
        } else {
            template = ich[TEMPLATE_BASE + "mutelist_set_row.html"](data);
            $(".add-url").val("");
        }
